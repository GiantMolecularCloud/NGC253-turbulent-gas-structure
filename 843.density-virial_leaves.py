...
for co,CO in lines.items():
    for gal,GAL in galaxies.items():
        ...
        bins       = fits[co][gal]['density-virial (leaves)']['size (astrodendro)']['linewidth (astrodendro)']['bins']
        ...
        coldens   = catalog['column density'][leaves]
        ...
        fig.savefig(os.path.join(plotdir, '05.coldens-sigma2size', 'binned_fit', 'coldens-sigma2size_binned_leaves.'+co+'.'+gal+'.pdf'), dpi=300, bbox_inches='tight')
...
mkpath(plotdir, '05.coldens-sigma2size', 'binned_fit')
fig.savefig(os.path.join(plotdir, '05.coldens-sigma2size', 'binned_fit', 'coldens-sigma2size_binned_leaves.all.pdf'), dpi=300, bbox_inches='tight')
...
for size_bin in tqdm(zip(bounds[:-1],bounds[1:])):
    ...
    for co,CO in lines.items():
        for gal,GAL in galaxies.items():
            ...
            leaves    = (catalog['type']=='leaf')
            size      = catalog['size (astrodendro)'][leaves].data
            linewidth = catalog['linewidth (astrodendro)'][leaves].data
            s2R       = linewidth**2/size
            coldens   = catalog['column density'][leaves].data
    ...
    ax.legend(bbox_to_anchor=(0.,1.02,1.,0.05), loc='lower left', mode='expand', borderaxespad=0., ncol=2, fancybox=True, fontsize=10)
    mkpath(plotdir, '05.coldens-sigma2size', 'binned_fit')
    fig.savefig(os.path.join(plotdir, '05.coldens-sigma2size', 'binned_fit', 'coldens-sigma2size_binned_leaves.all.'+str(size_bin[0])+'-'+str(size_bin[1])+'pc.pdf'), dpi=300, bbox_inches='tight')
