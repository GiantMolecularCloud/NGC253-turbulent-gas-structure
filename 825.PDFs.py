...
for co,CO in lines.items():
    for gal,GAL in galaxies.items():
        ...
        mkpath(plotdir, '06.PDF', 'volume_density')
        fig.savefig(os.path.join(plotdir, '06.PDF', 'volume_density', 'volume_density_PDF.'+co+'.'+gal+'.pdf'), dpi=300, bbox_inches='tight')
...
for co,CO in lines.items():
    for gal,GAL in galaxies.items():
        ...
        ax.set_axisbelow(True)
        ax.grid(ls=':', c='lightgrey')
        mkpath(plotdir, '06.PDF', 'mass')
        fig.savefig(os.path.join(plotdir, '06.PDF', 'mass', 'mass_PDF.'+co+'.'+gal+'.pdf'), dpi=300, bbox_inches='tight')
