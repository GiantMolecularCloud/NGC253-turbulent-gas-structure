...
for co,CO in lines.items():
    for gal,GAL in galaxies.items():
        ...
        ax.scatter(coldens, s2R, marker='.', s=10, c=color, label=co+' '+gal)
        ...
        for alpha in [1,10,100]:
            ax.plot([0.5*cd_min, 2.0*cd_max],
                    [(10/9*alpha*c.G*np.pi/5 *(2*u.u *r/u.cm**2) ).to(u.km**2/u.s**2/u.pc).value for r in [0.5*cd_min, 2.0*cd_max]],
                    ls=':', lw=1, color='darkgrey', zorder=1
                   )
            ax.text(0.5*cd_max,
                    1.7*(10/9*alpha*c.G*np.pi/5 *(2*u.u *5e22/u.cm**2) ).to(u.km**2/u.s**2/u.pc).value,
        ...
        mkpath(plotdir, '05.coldens-sigma2size', 'scatter')
        fig.savefig(os.path.join(plotdir, '05.coldens-sigma2size', 'scatter', 'coldens-sigma2size.'+co+'.'+gal+'.pdf'), dpi=300, bbox_inches='tight')
...
for co,CO in lines.items():
    for gal,GAL in galaxies.items():
        ...
        ax.scatter(coldens, s2R, marker='.', s=10, c=color,
                   label=co+' '+gal)
...
mkpath(plotdir, '05.coldens-sigma2size', 'scatter')
fig.savefig(os.path.join(plotdir, '05.coldens-sigma2size', 'scatter', 'coldens-sigma2size.all.pdf'), dpi=300, bbox_inches='tight')
...
for size_bin in tqdm(zip(bounds[:-1],bounds[1:])):
    ...
    for co,CO in lines.items():
        for gal,GAL in galaxies.items():
            ...
            ax.scatter(coldens, s2R, marker='.', s=10, c=color, label=co+' '+gal, zorder=3)
    ...
    ax.legend(bbox_to_anchor=(0.,1.02,1.,0.05), loc='lower left', mode='expand', borderaxespad=0., ncol=2, fancybox=True, fontsize=10)
    mkpath(plotdir, '05.coldens-sigma2size', 'scatter')
    fig.savefig(os.path.join(plotdir, '05.coldens-sigma2size', 'scatter', 'coldens-sigma2size.all.'+str(size_bin[0])+'-'+str(size_bin[1])+'pc.pdf'), dpi=300, bbox_inches='tight')
