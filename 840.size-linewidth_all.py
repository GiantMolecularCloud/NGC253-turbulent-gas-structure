...
for co,CO in lines.items():
    for gal,GAL in galaxies.items():
        ...
        fit        = fits[co][gal]['size-linewidth']['size (astrodendro)']['linewidth (astrodendro)']['fit']
        ...
        # fit
        x = np.logspace(np.log10(fits[co][gal]['size-linewidth']['fit range'][0]), np.log10(fits[co][gal]['size-linewidth']['fit range'][1]), 100)
        ...
        fig.savefig(join(plotdir, '04.size-linewidth', 'scatter_fit', 'size-linewidth.'+co+'.'+gal+'.pdf'), dpi=300, bbox_inches='tight')
...
mkpath(plotdir, '04.size-linewidth', 'scatter_fit')
fig.savefig(join(plotdir, '04.size-linewidth', 'scatter_fit', 'size-linewidth.all.pdf'), dpi=300, bbox_inches='tight')
...
for co,CO in lines.items():
    for gal,GAL in galaxies.items():
        ...
        dendrogram = dendrograms[co][gal]['dendrogram']
        color      = dendrograms[co][gal]['color']
        fit        = fits[co][gal]['size-linewidth']['size (astrodendro)']['linewidth (astrodendro)']['binned fit']
        bins       = fits[co][gal]['size-linewidth']['size (astrodendro)']['linewidth (astrodendro)']['bins']
        ...
        fig.savefig(join(plotdir, '04.size-linewidth', 'binned_fit', 'size-linewidth.'+co+'.'+gal+'.pdf'), dpi=300, bbox_inches='tight')
...
ax.legend(bbox_to_anchor=(0.,1.02,1.,0.05), loc='lower left', mode='expand', borderaxespad=0., ncol=2, fancybox=True, fontsize=10)
mkpath(plotdir, '04.size-linewidth', 'binned_fit')
fig.savefig(join(plotdir, '04.size-linewidth', 'binned_fit', 'size-linewidth.all.pdf'), dpi=300, bbox_inches='tight')
...
fig.savefig(join(plotdir, '04.size-linewidth', 'histogram', 'histograms.slope_sigma10.pdf'), dpi=300, bbox_inches='tight')
